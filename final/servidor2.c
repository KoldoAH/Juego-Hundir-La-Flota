 ...
 * DESCRIPCION: servidor del objetivo 2 de la Practica final.
 * gestiona la comunicacion entre dos clientes para que puedan 
 * jugar al juego hundir la flota, donde también 
...
int main(int argc,char* argv[]) {
	...
	int numeroMaximoObservadores= max_conexiones-2;
	int numeroObservadoresAhora=0;
	char tablaNombresObservadores[numeroMaximoObservadores][200];
	memset (tablaNombresJugadores,0,200);
	memset (tablaNombresObservadores,0,200);
	printf("num jugadores: %d\n", numeroJugadoresAhora);
	printf("num observadores: %d, num maximo observadores: %d \n", numeroObservadoresAhora, numeroMaximoObservadores);
			...
			if (FD_ISSET(i, &read_fds)){
				if (login_ok[i-3] == 0){ 
					...
					if (numeroJugadoresAhora<2){
							...
							for (int jug = 0; jug < 2; jug++){
								while(finaldepartida(tablero[(turno+1)%2]) != 0){
									...
									if (disparo == 1){ //tocado
										...
										strcat(info, "TOCADO ");
										strcat(info, tablaNombresJugadores[(turno)%2]);
										strcat(info, " ");
						...
						if (numeroObservadoresAhora<numeroMaximoObservadores){
							strcpy(tablaNombresObservadores[numeroObservadoresAhora], nombre);
							numeroObservadoresAhora= numeroObservadoresAhora+1;
							send(i,"S OBSERVADOR\n",sizeof("S OBSERVADOR\n"),0);
						}
						else{
							send(i,"S LLENO\n",sizeof("S LLENO\n"),0);
